    ...
    <nav className="bg-white text-black border-b border-gray-100 sticky top-0 z-50 shadow-sm">
        ...
        <div className="hidden md:flex gap-6">
          ...
          {user && (
            <Link
              ...
              className="flex items-center gap-1 text-[#484848] font-medium hover:bg-blue-50 hover:text-[#00A5EC] transition p-5"
            >
              Messages
            </Link>
          )}
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-t border-gray-100 flex flex-col pb-4">
          <Link
            to="/jobs"
            aria-label="Browse available jobs mobile link"
            className="p-4 border-b border-gray-50 text-[#484848]"
          ...
          <Link
            to="/jobs?type=internship"
            aria-label="Browse available internships mobile link"
            className="p-4 border-b border-gray-50 text-[#484848]"
            onClick={() => setIsOpen(false)}
          >
            Internships
          ...
          {token && role === "student" && (
            <Link
              to="/profile"
              aria-label="View student profile mobile link"
              className="p-4 border-b border-gray-50 text-[#484848]"
              onClick={() => setIsOpen(false)}
            >
              Profile
          ...
          {token && role === "employer" && (
            <Link
              to="/employer-dashboard"
              aria-label="View employer dashboard mobile link"
              className="p-4 border-b border-gray-50 font-bold text-[#484848]"
              onClick={() => setIsOpen(false)}
            >
              Employer
            </Link>
          ...
          <div className="p-4 flex flex-col gap-3">
            {!token && (
              <>
                <Link
                  to="/login"
                  aria-label="Log in to your account mobile link"
                  className="w-full py-3 text-center text-[#008BDC] border border-[#008BDC] rounded font-semibold"
                  onClick={() => setIsOpen(false)}
                >
                  Login
                </Link>
                <Link
                  to="/register"
                  aria-label="Create a new account mobile link"
                  className="w-full py-3 text-center bg-[#00A5EC] text-white rounded font-semibold"
                  onClick={() => setIsOpen(false)}
                >
                  Register
                </Link>
